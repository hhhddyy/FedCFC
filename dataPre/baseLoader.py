...
import random
import pickle
from dataPre.utils import Normalizer
...
class BASE_DATA():
    ...
    def update_train_val_test_keys(self):
        ...
        act_weights = class_weight.compute_class_weight('balanced', classes=range(len(self.no_drop_activites)),y=y_of_all_windows)
    ...
    def genarate_spectrogram(self):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        self.freq_path = os.path.join(save_path, "diff_{}_window_{}_step_{}".format(self.difference, self.windowsize,
                                                                                    self.displacement))
        if os.path.exists(self.freq_path):
            ...
            for window in self.slidingwindows:
                ...
                for j in range(sample_x.shape[1]):
                    [cwtmatr, frequencies] = pywt.cwt(sample_x[:, j], scales, self.wavename,
